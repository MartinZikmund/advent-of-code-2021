...
var pathCount = GetPathCount(start, new List<string>() { "start" }, false);
Console.WriteLine(pathCount);

long GetPathCount(Cave current, List<string> visited, bool didDoubleVisit)
{
    if (current.Name == "end")
    {
        return 1;
    ...
    foreach (var connection in current.Connections)
    {
        ...
        if (char.IsUpper(connection.Name[0]) || !visited.Contains(connection.Name) || !didDoubleVisit)
        {
            var secondVisit = didDoubleVisit || (char.IsLower(connection.Name[0]) && visited.Contains(connection.Name));
            visited.Add(connection.Name);
            totalPaths += GetPathCount(connection, visited, secondVisit);
